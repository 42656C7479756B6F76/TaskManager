...
with recursive task_hierarchy as (
    select id as task_id
         ...
         , title
         , status
         , array[]::bigint[] as parent_task_ids
    from tasks
    ...
    select t.id as task_id
         ...
         , t.title
         , t.status
         , parent_task_ids || t.parent_task_id
    from task_hierarchy th
    join tasks t on t.parent_task_id = th.task_id
)   
   select task_id
        , title
        , status
...
";
        ...
        if (statuses.Any())
        {
            conditions.Add("and status = any(@StatusIds)");
            @params.Add("StatusIds", statuses);
        }
        
        var cmd = new CommandDefinition(
            baseSql + $" {string.Join(" and ", conditions)} ",
            @params,
            commandTimeout: DefaultTimeoutInSeconds,
            cancellationToken: token);
